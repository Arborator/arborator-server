...
def printmyassignments(project,sql,projectconfig,userid): # assigned texts
	...
	for _,_,tid,todotype,status,comment in sql.getall(None, "todos",["userid"], [userid]):
		try:
			...
			sql.removeText(tid)
			txt += u"we found a todo for a non existing text and removed it."
			continue
		try: todotype=int(todotype)
		except: todotype=0
		
		
		if todotype:txt += u"	<tr><td><b><span class='val'>validation of <br></span><a  style='cursor:pointer;' href='"+url_for('editor',project=project,text_id=tid,opensentence=1)+"'>"+ t +"</a></b></td>"
		else:	txt += u"	<tr><td><b><a  style='cursor:pointer;' href='"+url_for('editor',project=project,text_id=tid,opensentence=1)+"'>"+ t +"</a></b></td>"
	...
	if scount: return txt
	else: return ""
	#else: return u"""<div class="ui-widget ui-widget-content ui-corner-all box"  style="text-align:-moz-center;">
	#You have not been assigned any text of the <b>{project}</b> Project<br/></div>
	#""".format(project=project).encode("utf-8")


def printalltexts(project,sql,adminLevel, projectconfig):############################# all texts of the project
	...
	for t,tid,nrtokens in sorted( [(t,tid,nrtokens) for tid,t,nrtokens in sql.getall(None, "texts",None,None)]): # got to get it in alphabetical order .decode("utf-8")
		...
		c1= u"<b><a  style='cursor:pointer;dispay:block'  href='{tid}'>{t}</a></b></td>".format(tid=url_for('editor',project=project,text_id=tid,opensentence=1), t=t)
		sn=sql.getnumber(None, "sentences", ["textid"], [tid])
	...
	for tup in tuples:
		txt += ('<tr><td>' + u"</td><td>".join(tup) + "</td></tr>")
	...
	if nrt: # at least some texts exist
		...
		txt += """<TFOOT>	<tr style='padding-top:20;'><td>{nrt} texts</td><td>{stotal} sentences<br>{nvs} validated</td><td>{ttotal} tokens<br>{nvt} validated</td><td>{avsentlen}<br>{avsentlenval} validated</td><td>{annota}</td><td>{validata}</td><td>&nbsp;</td><td>{evallinkhtml}</td><td>&nbsp;</td><td>{allexport}&nbsp;</td><td>&nbsp;</td></tr> 	</TFOOT>""".format(stotal=stotal, nvs=nvs, nvt=nvt, nrt=nrt, ttotal=ttotal,avsentlen=avsentlen, avsentlenval=avsentlenval, annota=annota,validata=validata,evallinkhtml=evallinkhtml if adminLevel else "&nbsp;", allexport=allexport)
	txt+= "</table></div>"
	return txt
...
def printuserassignments(project,projectconfig,sql,adminLevel, lastseens=[]):
	...
	for uid,u,rea in sql.getall(None, "users",None,None, orderby="order by realname"):
		if u!=parser:
			...
			if teacher:
				...
				tablecontent+= "<tr><td>{rea} </td><td colspan=5>".format(rea)
	...
	if adminLevel>1:
		txt+= """<div class="ui-widget ui-widget-content ui-corner-all box">
...
def printmenues(project,sql, projectconfig):
	
	parser=projectconfig["configuration"]["importAnnotatorName"]
	txt = """<form method="get" action="editor.cgi" style="display: none;" id="editorform" >
			<input type="hidden" id="project" name="project" value="{project}">
	...
	txt +=  """<form method="post" action="project.cgi?project={project}" style="display: none;" id="nextstatusform" >
			<input type="hidden" id="statustextid" name="statustextid" value="">
		</form>
	""".format(project=project)
	#<input type="hidden" id="project" name="project" value="{project}">
	
	txt +=  """<div id="exoform" style=" position:absolute;display: none;" onclick="var e=arguments[0];e.stopPropagation();">
	...
	txt +=  """<div id="userform" style="position:absolute; display:none;" onclick="var e=arguments[0];e.stopPropagation();">
	...
	for row in sorted(sql.getall(None, "ausers"), key=lambda x: x[2].lower()):
			print row
			if row[1]!=parser:
				#print [user]
				txt +=  "			<option>"
	...
	txt +=  """<div class="ui-widget ui-widget-content ui-corner-all" style="position: absolute; padding: 5px;z-index:20;display:none;font-size:.8em;" id="exportdiv">
