...
class SQL:
	...
	def enterTree(self, cursor, nodedic, sentenceid, userid, notenter=[], intokenname=None, tokensChanged=False):
		...
		for i in sorted(nodedic.keys()): # for every token
			...
			# checking if defaults are needed
			if self.catName in notenter:
				self.enter(cursor, "features",["treeid","nr","attr","value"],(treeid,i,self.catName,"_",), computeId=False)
			if "gov" in notenter:
				self.enter(cursor, "links",["treeid","depid","govid","function"],(treeid,i,-1,"_",), computeId=False)
	...
	def links2AllTrees(self,sid,snr,username,adminLevel, todo=-1, validvalid=[],validator=False, addEmptyUser=None):
		...
		if not treeDict or (addEmptyUser and addEmptyUser not in treeDict and username not in treeDict):
			...
			if nodedic: # found tree of self.exoBaseAnnotatorName, save it as user's tree
				...
				if exoBaseAnnoId:
					# if there's such a thing as a baseAnnotator, we add an empty tree for her
					self.enterTree(cursor, nodedic, sid, exoBaseAnnoId, notenter=["gov",self.catName,"tag2"])
				# and we also enter the tree for our user (with defaults "tag":"_", "gov":{-1:"_"})
	...
	def visibleTrees(self, id_to_copy_from=1):
		"""
		Used to resolve the "invisible trees" problem.

		Copies the tree from another user (by default "parser") and add default
		pos tag and gov.
		"""
		db,cursor=self.open()
		trees= cursor.execute("""select trees.rowid, trees.sentenceid, trees.userid, users.user from trees, users WHERE trees.userid=users.rowid;""")
		for treeid,sid,uid,uname in trees.fetchall():
			a = self.gettree(sid=sid, uid=uid, indb=db, incursor=cursor)["tree"]

			if not a: # the returned tree is empty for the user

				# copy the parser tree and change (0,root) to (-1,_)
				parser_tree = self.gettree(sid=sid,uid=id_to_copy_from,indb=db,incursor=cursor)["tree"]
				for i in sorted(parser_tree.keys()):
					parser_tree[i]["gov"] = {-1:"_"}
				# add it under the user
				self.enterTree(cursor, parser_tree, sid, uid)
