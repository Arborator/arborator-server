	...
	if eraseAllAnnos:
		...
		for i,nodedic in enumerate(sentences): # for every sentence
			...
			if len(nodedic)<maxlength:
				...
				if debug:print "----",scounter,"of",len(nodedic),sent.encode("utf-8")," - it took",time()-ti,"seconds up to now for",wcounter, "words - ", float(wcounter)/(time()-ti),'words per second.'
...
#def entersinglerhapsodieannotation(projectname,rhapsodiexmlfile,repair=False):
	#sql = SQL(projectname)
	#annotatorName=sql.projectconfig["configuration"]["baseAnnotatorName"]
	#eraseAllAnnos=True
	#if repair:corrdic,gluedic=correctionDics(repair)
	#else: corrdic,gluedic=False,False
	#enterRhapsodie(sql,rhapsodiexmlfile,annotatorName, eraseAllAnnos,clean=True,corrdic=corrdic,gluedic=gluedic)
	#print "ok"

		
	#def enterRhapsodie(sql,inputfilename, annotatorName=None, eraseAllAnnos=False, dbtextname=None,  clean=True, corrdic=False, gluedic=False):		
		
		#print "______________________________________________________________reading",inputfilename
		
		#if clean: print sql.cleanDatabase(inputfilename)
		
		#db,cursor=sql.open()
		
		#if corrdic:
			
			#sentences, sentencefeatures=correctRhapsodie(inputfilename, corrdic,gluedic)
		#else:
			#sentences, sentencefeatures=rhapsodie2Sentences(inputfilename)
			
			
		#if not annotatorName: annotatorName=sql.projectconfig["configuration"]["baseAnnotatorName"]
		#if not dbtextname: dbtextname=inputfilename.split("/")[-1]
		#textid = sql.enter(cursor, "texts",["textname"],(dbtextname,))
		#print "______________",len(sentences)," sentences ready to enter into database"
		#enterSentences(sql,cursor,sentences,inputfilename, textid,annotatorName,eraseAllAnnos, sentencefeatures=sentencefeatures )
		##enterSentences(sql,sentences,inputfilename, parser,eraseAllAnnos )
		##enterSentences(sql,sentences,inputfilename)
		#db.commit()
		#db.close()
		#return len(sentences)	
...
if __name__ == "__main__":
	...
	#readinsinglerhapsodie("Rhapsodie","/home/kim/Dropbox/Rhapsodie/syntaxe-julie/xml_ok/new_name/Rhap-D2010-Synt.xml",repair="/home/kim/Dropbox/Rhapsodie/syntaxe-julie/settings/spacelemmas7.txt")
	#Rhap-D2010-Synt.xml
	#readinsinglerhapsodie("Rhapsodie","/home/kim/Dropbox/Rhapsodie/syntaxe-julie/xml_ok/D003.XML",repair="/home/kim/Dropbox/Rhapsodie/syntaxe-julie/settings/spacelemmas6.txt")
	...
	#sql.enterConll("corpus/conll/xinying6.csv.112.conll10")
