...
def tree2nodedic(tree, correctiondic={}, rhaps=False):
	...
	for line in tree.split('\n'):
		...
		if line.strip():
			...
			if not rhaps and nrCells in [4,10,13,14,19]:
				if nrCells == 4: # malt!
			...
			elif rhaps==True and nrCells in [3,5,15]:
...
def makeEmpty(infilename,removeHash=True):
	"""
	...
	if infilename.endswith(".trs.lif.w+p.orfeo"):
		emptyname=infilename[:-len(".trs.lif.w+p.orfeo")]+"-one-word-per-line.conll14"
	elif infilename.endswith(".trs.lif.w+p_anon.orfeo"):
		emptyname=infilename[:-len(".trs.lif.w+p_anon.orfeo")]+"-one-word-per-line.conll14"
		
	
	else:
		emptyname=infilename+"-one-word-per-line.conll14"
	with codecs.open(emptyname,"w","utf-8") as emptyfile:
		for tree in conll2trees(infilename):
			for i in sorted(tree.keys()):
				...
				emptyfile.write("\t".join([str(node["id"]),t, "_", "_", "_", "_", "_", "_", "_","_","_","_","_","_"])+"\n")
			emptyfile.write("\n")
