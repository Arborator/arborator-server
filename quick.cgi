...
def editprinthtmlheader(project,projectconfig,textid,textname,username,userid,validator,opensentence):

	print """<html><head>
		<title>Arborator - Editor - """+textname.encode("utf-8")+"""</title>
...
def printhtmlheader(project,projectconfig):
...
def printforms(conlltext,projectconfig):
	...
	print """<span>Paste CoNLL file below or type a sentence here:<input type='text' id='sentence' placeholder="your sentence" style='width:55%' >  </input> 
	<button style="float:right;z-index:33;" onclick="conllize();" role="button" class="fg-button ui-state-default ui-corner-all ui-state-enabled" id="button"><span class="ui-button-text">sentence to simple CoNLL format</span></button></span>
	"""
	print """<TEXTAREA NAME="conll" id="conll" style="font: normal 10px Sans-Serif;width:100%; height:200;" """,
...
4	simple	simple	Adj	_	_	2	pred	_	_"""
	...
	print "<div ><span style='float:left;'>functions</span><input type='text' id='addfuncs'  style='width:93%;float:right;' value='{functions}'></input> </div>".format(functions=" ".join(projectconfig.functions))
	print "<div ><span style='float:left;'>categories</span><input type='text' id='addcats'  style='width:93%;float:right;' value='{categories}'>  </input> </div>".format(categories=" ".join(projectconfig.categories))
	
	print "<br><br>"
	print """<script type="text/javascript">
		$("textarea,input").focus(function () {nokeys=true;}).blur(function () {nokeys=false;console.log("lost focus")});
		</script>
	"""
...
def start(project):
	...
	return projectconfig,thisfile,opensentence,conlltext
...
if __name__ == "__main__":
	project="quickie"
	projectconfig,thisfile,opensentence,conlltext=start(project)
	printhtmlheader(project,projectconfig)
	printheadline("Quickedit")
	
	printexport()
	
	printforms(conlltext,projectconfig)
