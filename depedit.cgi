...
def start():
	...
	loggedin = isloggedin('users/')
	projectconfig=None
	
	if loggedin:
		#print loggedin
		return thisfile, loggedin[0]["username"].decode("utf-8"), int(loggedin[0]["admin"])		
	else:
		#print "user not logged in"
		return thisfile,None, None
...
def printmate(project,projectconfig, sql, adminLevel):
	...
	<div class="ui-widget ui-widget-content ui-corner-all box" style="text-align:-moz-center;">Mate:
		...
		<form  method="post" id="startdepeditdatabaseform" name="startdepeditdatabaseform">
			...
			<br/>
			<input type="button" cursor="pointer" id="buttonstartdepeditdatabase" name="buttonstartdepeditdatabase" value="start Mate" style="width:155px;z-index:33;" class="fg-button ui-state-default ui-corner-all" onclick="startdepeditdatabase();">
...
adminDiv=u"""
	<div class="ui-widget ui-corner-all box" style="text-align:-moz-center;">
		<form  method="post" id="applyToDatabaseForm" name="applyToDatabaseForm">
			Apply the grammar to <input type="hidden" id="etextid" name="textid" value="">
				<select id="projects" name="projects" style="width:100px;height:25px;"  >
					{projects}
				</select>
			and add this string to name of the new project <input id="addName" type="text" value="_{username}" name="addName" title="additional name of new project" style="width:133px;">.
			<br/>
			<input type="button" cursor="pointer" id="buttonstartdepeditdatabase" name="buttonstartdepeditdatabase" value="Apply Depedit to Database" style="z-index:33;" class="fg-button ui-state-default ui-corner-all" onclick="applyGrammar();">
		</form>
		<div id="parseStatus" class="ui-widget ui-widget-content ui-corner-all box" style="background-position: center center;background-repeat:no-repeat;">
		</div>
	</div>&nbsp;<br/>&nbsp;"""
	
	#class="ui-widget ui-widget-content ui-corner-all box" style="text-align:-moz-center;"
	#<input type="radio" name="whoseTrees" value="v" checked>the validators' trees <input type="radio" name="whoseTrees">
	#Use <input id="additionnalLexicon" type="text" value="" name="additionnalLexicon" title="name of additional lexicon to use" style="width:93px;"> as additional lexicon and store the parse results under
	#<input id="resultAnnotator" type="text" value="mate" name="resultAnnotator" title="username for resulting trees" style="width:63px;">.
	#<br/>
...
if __name__ == "__main__":
	...
	thisfile,username,adminLevel = start()
	if adminLevel:
		applyToDatabase=adminDiv.format(projects="\n".join(["<option>"+ty+"</option>" for ty in sorted(config.projects, key=unicode.lower)]), username=username)
	else:
		applyToDatabase=""
	html = codecs.open("depedit.html","r","utf-8").read().replace(u"{applyToDatabase}",applyToDatabase)
