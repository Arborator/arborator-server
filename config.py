#!/usr/bin/python
# -*- coding: utf-8 -*-
import os, codecs, json, colorsys,random
...
def configProject(projectName):
	...
	filename=os.path.join(unicode(os.getcwd()),u"projects",unicode(projectName),u"project.cfg" ).encode("utf-8")
	if projectName not in projects: 
		print 'Content-type: text/plain\n\nxxxxxxxxxxxxxxxxxxxx',"error in projects:",type(projectName),[projectName]
		print projects
		return
...
def readinContent(config,projectName, tmp=""):
	...
	fn=os.path.join(os.getcwd(),"projects",projectName.encode("utf-8"),(config["configuration"]["functionsfilename"]+tmp).encode("utf-8") ) 
	...
	#config.categories={}
	#try:
	fn=os.path.join(os.getcwd(),"projects",projectName.encode("utf-8"),(config["configuration"]["categoriesfilename"]+tmp).encode("utf-8") )  
	f=codecs.open(fn,"r","utf-8")
	for li in f:
		if li.strip(): 
...
def checkConfigProject(projectName, filename=u"project.cfg",readin=True,tmp=""):	
	filename=unicode(os.path.join(unicode(os.getcwd()),u"projects",projectName,filename )).encode("utf-8")
	config = ConfigObj(filename,encoding="UTF-8")
