...
def reverseauxEnTete(tree):
	"""
	aux est la tête
	"""
	for i, node in tree.iteritems():
		if node["gov"] and node["gov"].values()[0] in ["aux"] and node["gov"].keys()[0]<i: # found a pp
			#print "found a node that is dependent on aux",i,node
			gi = node["gov"].keys()[0]
			gnode=tree[gi]
			#print "gnode",gi,gnode
			...
			if gnode["gov"] and gnode["gov"].values()[0] in ["aux"]:# and gnode["gov"].keys()[0]<gi: # found a double aux
				...
				if "aux" not in node["children"].values():
...
def reverseauxpara(tree):
	"""
	#avant:si X-para-> Y et X-aux-> Z, alors Y-aux-> Z
	#si X-para-> Y et Y-aux-> Z, alors X-aux-> Z
	"""
	for i, node in tree.iteritems():
		if "para" in node["gov"].values() and "aux" in node["children"].values():
			#print "found a node X-para-> Y et Y-aux-> Z. Y:",i,node
			xparai=[ci for ci in node["gov"] if node["gov"][ci]=="para"][0]
			zauxi =[ci for ci in node["children"] if node["children"][ci]=="aux" ][0]
			tree[zauxi]["gov"]={i:"aux"}
			addinfototree(tree)
			#pprint(tree)
			#qsdf
	return  tree	
...
def reversecollapsecomps(tree):
	"""
	
	"""
	for i, node in tree.iteritems():
		for gi,f in node["gov"].iteritems():
			if f in "comp aff ad".split():
				node["gov"]={gi:"dep"}
...
def platinum(tree):
	...
	#tree=rendezvous(tree)
	#tree=pasgrandchose(tree)
	tree=reverseauxpara(tree)
	tree=reverseauxEnTete(tree)
	tree=reversecollapsecomps(tree)
	#print "after aux:"
	#pprint (tree)
	#tree=desEnTete(tree)
	#tree=adperiph(tree)
...
def transform(infolder,outfolder,mixOldNew=True, filepattern="*.conll"):
	createNonExistingFolders(outfolder)
	spaceToks={}
	#for infile in sorted(glob.glob(os.path.join(infolder,"test.conll"))):
	for infile in sorted(glob.glob(os.path.join(infolder,filepattern))):
