...
def filltemplate(template, formdict=None, vallist=None):
    ...
    for entry in keynames+vallist:
        if entry in ['pass1', 'pass2']:
           continue
        # entry is for example "username"
        # kim's add on:
        template= template.decode("utf-8")
        #a=template.replace(             u'**%s**' % entry,    formdict.get(entry, '').decode("utf-8"))
        template = template.replace(    u'**%s**' % entry,    formdict.get(entry, '').decode("utf-8")    ).encode("utf-8")
    return template
